  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 px-4 sm:px-6 md:px-10 py-6 w-full">
      <aside className="w-full lg:w-[280px] flex-shrink-0">
        <h3 className="text-gray-700 text-[16px] font-semibold mb-4">
          Filtrar por:
        </h3>
      ...
      <main className="flex-1 w-full">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
          <span className="text-sm text-gray-500">
            {filters.length === 0 ? (
              <>
                Todos os produtos – <strong>{processedProducts.length}</strong>{" "}
                produto(s)
              </>
            ) : (
              <>
                Resultados para <strong>{filters.join(", ")}</strong> –{" "}
                <strong>{processedProducts.length}</strong> produto(s)
              </>
            )}
          </span>
